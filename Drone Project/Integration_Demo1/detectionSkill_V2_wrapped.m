function [ballPosition, selectedLines, ballStatusFlag, lineStatusFlag, outputFrame]=detectionSkill_V2_wrapped(ballSelectionMode, lineDetectionEnabled, inputFrame)
...
% Function exclusion from code generation
coder.extrinsic('imresize','im2bw','imfindcircles','hough','houghpeaks','houghlines')
...
%         end
    ...
    case 2 % Using Color Based Detector
            ...
            if numPts < 10
                ...
                % Color Detection
                [BW,maskedI]=createHsvYellowMask(videoFrame); % Edit the color mask function if needed
                ...
                radii = 0;
                metric = 0;
                [centers, radii, metric] = imfindcircles(I,[60 100], 'Sensitivity',0.95);
                ...
                if ~isempty(centers)
                    ...
                    if bbox(1)+2*radiiS>frameSize(2)||bbox(2)+2*radiiS>frameSize(1)||bbox(1)-2*radiiS>frameSize(2)<0||bbox(2)-2*radiiS>frameSize(1)<0
                        zbbox=0;
                    elseif bbox(1)>0&&bbox(2)>0
                        zbbox = 1; ballStatusFlag=1;
    ...
    otherwise
        ...
        ballPosition=[0,0];
        selectedLines=zeros(SumCandidates,6);
        ballStatusFlag=0;
        lineStatusFlag=0;
end           
